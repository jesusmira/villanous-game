...
export function CardComponent({ card, state, selected, onClick, onDetailClick, small }: Props) {
  ...
  return (
    <div
      ...
      style={{ '--card-color': TYPE_COLORS[card.cardType] ?? '#333' }}
      onClick={onClick}
      title={card.name}
    >
      ...
      {!small && desc && <div className="card-desc">{desc}</div>}
      {effectiveStr !== undefined && <div className="card-strength">{effectiveStr}</div>}
      {onDetailClick && (
        <button
          className="card-info-btn"
          onClick={e => { e.stopPropagation(); onDetailClick(card); }}
          title="Ver detalles"
        >?</button>
      )}
    </div>
